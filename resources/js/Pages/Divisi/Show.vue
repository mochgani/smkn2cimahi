...
const props = defineProps({
    divisi:     { type: Object, required: true },
...
const breadcrumbMap = {
    'kurikulum':         [{ label: 'Kurikulum' }],
    'kesiswaan':         [{ label: 'Kesiswaan' }, { label: 'Program Kesiswaan' }],
    'hubungan-industri': [{ label: 'Hubungan Industri' }],
};

const breadcrumbs = [{ label: 'Beranda', href: '/' }, ...(breadcrumbMap[props.divisi.slug] ?? [{ label: props.divisi.name }])];

const titleMap = {
    'kurikulum':         'Info & kabar dari Divisi Kurikulum.',
    'kesiswaan':         'Program & kegiatan Divisi Kesiswaan.',
    'hubungan-industri': 'Info & kerjasama Divisi Hubungan Industri.',
};

const pageTitle = titleMap[props.divisi.slug] ?? `Berita dari Divisi ${props.divisi.name}.`;
</script>

<template>
    <Head :title="divisi.name" />

    <AppLayout>
        <PageHeader
            :breadcrumbs="breadcrumbs"
            :meta="`${totalCount} ARTIKEL · DIVISI ${divisi.name.toUpperCase()}`"
            :title="pageTitle"
            :lead="divisi.description || `Berita, kegiatan, dan informasi terbaru dari Divisi ${divisi.name} SMK Negeri 2 Cimahi.`"
        />
                ...
                <div class="grid grid-cols-1 md:grid-cols-[1.4fr_1fr]">
                    ...
                    <div class="order-1 md:order-2 relative min-h-[200px] sm:min-h-[280px] md:min-h-[400px] bg-line-soft overflow-hidden">
                        ...
                        <div class="absolute bottom-3 left-3 sm:bottom-6 sm:left-6 bg-white/95 px-3 sm:px-5 py-2 sm:py-3.5 border border-line">
                            <div class="font-mono text-[9px] sm:text-[10px] text-muted tracking-mono-wide mb-1">FEATURED</div>
                            <div class="font-mono text-[12px] sm:text-sm font-semibold text-ink tracking-mono">
                                {{ divisi.name.toUpperCase() }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <section class="container-page py-6 sm:py-8">
            <SectionLabel num="1" :title="`Berita Divisi ${divisi.name}`" />
            ...
            <div v-else-if="!featured" class="py-20 text-center text-muted font-mono text-sm border border-dashed border-line">
                Belum ada berita dari Divisi {{ divisi.name }}.
            </div>
